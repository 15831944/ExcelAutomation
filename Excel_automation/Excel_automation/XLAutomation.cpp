...
BOOL CXLAutomation::SaveAs(CString szFileName, int nFileFormat, CString szPassword, CString szWritePassword, BOOL bReadOnly, BOOL bBackUp)
{
	...
	//hResult = AutoWrap(DISPATCH_PROPERTYPUT, NULL, m_stProperties.pXlApp, L"DisplayAlerts", 1,0);

	VARIANT x;
	x.vt = VT_BOOL;
	x.boolVal = VARIANT_FALSE;
	
	// DisplayAlerts«×«í«Ñ«Æ«£ªÎàâïÒ
	AutoWrap(DISPATCH_PROPERTYPUT, NULL, m_pdispExcelApp, L"DisplayAlerts", 1, x);

	if (!ExlInvoke(m_pdispWorkbook, L"Save", NULL , DISPATCH_METHOD, DISP_FREEARGS))
		return FALSE;
...
HRESULT CXLAutomation::AutoWrap(int nAutoType, VARIANT *pvResult, IDispatch *pDisp, LPOLESTR ptName, int cArgs...) 
{
    va_list vlMarker;
    ...
    if(FAILED(hResult)) 
	{
       // PrintOut(L"IDispatch::GetIDsOfNames(\"%s\") ½ÇÆÐ w/err 0x%08lx", szName, hResult);
        return FALSE;
    }
    ...
    if(FAILED(hResult))
	{
		//PrintOut(L"IDispatch::Invoke(\"%s\"=%08lx) ½ÇÆÐ w/err 0x%08lx", szName, hResult);
		switch(hResult)
		{
		case DISP_E_BADPARAMCOUNT:
